...
suite('Nacha Parser Validation Test Suite', () => {
    ...
    test('Should detect short record length', () => {
        ...
        const lengthErrors = diags.filter(d => d.code === 'ACH-PHYSICAL-RECORD-LENGTH');
    ...
    test('Should validate routing number checksum', () => {
        ...
        const makeEntry = (checkDigit: string) => '622' + '06100010' + checkDigit + pad('123456789', 17) + '0000001234' + pad('ID12345', 15) + pad('Individual Name', 22) + '  ' + '0' + '061000100000001';
        const validEntry = makeEntry('4');
        const invalidEntry = makeEntry('5');
        ...
        const checksumErrorsValid = diagsValid.filter(d => d.code === 'ACH-FIELD-ROUTING-CHECK-DIGIT');
        const checksumErrorsInvalid = diagsInvalid.filter(d => d.code === 'ACH-FIELD-ROUTING-CHECK-DIGIT');
    ...
    test('Should validate batch totals', () => {
        ...
        const fullFile = [fileHeader, batchHeader, entry, batchControl, fileControl, ...Array(5).fill('9'.repeat(94))].join('\n');

        const diags = parseAch(fullFile);
        const totalErrors = diags.filter(d => d.category === 'relational');
    ...
    test('Should validate IAT records', () => {
        ...
        const fullFile = [fileHeader, batchHeader, entry, addenda10, addenda11, addenda12, addenda13, addenda14, addenda15, addenda16, batchControl, fileControl, ...Array(8).fill('9'.repeat(94))].join('\n');

        const diags = parseAch(fullFile);
        const totalErrors = diags.filter(d => d.category === 'relational' || d.code.startsWith('ACH-IAT'));
